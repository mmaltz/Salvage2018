...
metassu$Site<-as.factor(metassu$Site)
...
comm.grps<-ssuspeciesREADRICHlong%>%dplyr::select('ID','Location','Year','Site','Treat','Rep','TSDel','HL','Description') #mapping data
colnames(comm.grps)
```
###WORK ON THIS!!!
Make the comm.mat 
...
subset data by year
```{r ss by year}
Site2017.mat=comm.mat[comm.grps$Year=="2017",]
Site2017.grps=comm.grps[comm.grps$Year=="2017",]
Site2017.bc<-vegdist(Site2017.mat, method='bray', na.rm=TRUE)

Site2017.grps$Treat=droplevels(Site2017.grps$Treat)
Site2017.grps$Site<-as.factor(Site2017.grps$Site)

Site2017.div<-adonis2(Site2017.bc~Treat, strata= Site, data=Site2017.grps, permutations = 999, method="bray")
Site2017.div

Site2017.div<-adonis2(Site2017.bc~Treat+Site, data=Site2017.grps, permutations = 999, method="bray")
Site2017.div
```
Subset data from year for NMDS
```{r}

YearMDS<-metaMDS(Site2017.mat, distance="bray", k=2, trymax=35, autotransform=TRUE) ##k is the number of dimensions
YearMDS ##metaMDS takes eaither a distance matrix or your community matrix (then requires method for 'distance=')
...
NMDS1 <- YearMDS$points[,1] ##also found using scores(amfMDS)
NMDS2 <- YearMDS$points[,2]
?cbind
YearMDS.plot<-cbind(Site2017.grps, NMDS1, NMDS2, Site2017.mat)

t<-ggplot(YearMDS.plot, aes(NMDS1, NMDS2, color=Treat))+
  geom_point(position=position_jitter(.1), shape=3)+##separates overlapping points
  stat_ellipse(type='t',size =1)+ ##draws 95% confidence interval ellipses
  theme_minimal()
t

s<-ggplot(YearMDS.plot, aes(NMDS1, NMDS2, color=Site))+
  geom_point(position=position_jitter(.1), shape=3)+##separates overlapping points
  stat_ellipse(type='t',size =1)+ ##draws 95% confidence interval ellipses
  theme_minimal()
s

Ts<-ggplot(YearMDS.plot, aes(NMDS1, NMDS2, color=Treat))+
  geom_point(position=position_jitter(.1), shape=Site2017.grps$Site)+##separates overlapping points
  stat_ellipse(type='t',size =1)+ ##draws 95% confidence interval ellipses
  theme_minimal()
Ts
...
Code to subset by site (after year)
```{r}
PS2017.mat=Site2017.mat[Site2017.grps$Site=="PS",]
PS2017.grps=Site2017.grps[Site2017.grps$Site=="PS",]
PS2017.bc<-vegdist(PS2017.mat, method='bray', na.rm=TRUE)

#PS2017.grps$Treat=droplevels(Site2017.grps$Treat)
#PS2017.grps$Site<-as.factor(Site2017.grps$Site)

PS2017.div<-adonis2(PS2017.bc~Treat, data=PS2017.grps, permutations = 999, method="bray")
PS2017.div

PSHL2017.div<-adonis2(PS2017.bc~Treat+HL, data=PS2017.grps, permutations = 999, method="bray")
PSHL2017.div


##NMDS to edit
PS17MDS<-metaMDS(PS2017.mat, distance="bray", k=2, trymax=35, autotransform=TRUE) ##k is the number of dimensions
PS17MDS ##metaMDS takes eaither a distance matrix or your community matrix (then requires method for 'distance=')
...
NMDS1 <- PS17MDS$points[,1] ##also found using scores(amfMDS)
NMDS2 <- PS17MDS$points[,2]
?cbind
PS17MDS.plot<-cbind(PS2017.grps, NMDS1, NMDS2, PS2017.mat)

t<-ggplot(PS17MDS.plot, aes(NMDS1, NMDS2, color=Treat))+
  geom_point(position=position_jitter(.1), shape=3)+##separates overlapping points
  stat_ellipse(type='t',size =1)+ ##draws 95% confidence interval ellipses
  theme_minimal()
t
...
Code from the Test28 rmd file pasted in here to evaluate by species:
